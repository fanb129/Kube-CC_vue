<template>
  <div class="login-container">
    <el-form ref="form" :model="form" :rules="registerRules" class="login-form" auto-complete="on" label-position="left">
      ...
      <el-button :loading="loading" type="primary" style="width: 30%" @click.native.prevent="handleFindPass">提交</el-button>
...
<script>
import {captcha, checkcp, emailcaptcha, findPass} from '@/api/user'
    ...
    handleFindPass() {
      if (this.form.emailcode === '') {
        ...
        this.$refs.form.validate(valid => {
          if (valid) {
            this.loading = true
            findPass({ password: this.form.password, email: this.form.email, vcode: this.form.emailcode }).then(() => {
              Message({
                message: '密码修改成功',
                type: 'success',
                duration: 3 * 1000
